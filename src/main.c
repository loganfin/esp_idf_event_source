...
static esp_err_t infinite_loop(httpd_req_t* req)
{
    // will always be called by the worker task
...
static esp_err_t sse_handler(httpd_req_t* req)
{
    ...
    // submit
    if (submit_async_req(req, infinite_loop) == ESP_OK) {
        return ESP_OK;
    } else {
        httpd_resp_set_status(req, "503 Busy");
        httpd_resp_sendstr(
...
esp_err_t session_open_callback(httpd_handle_t hd, int sockfd)
{
    ESP_LOGI(TAG, "New session with sockfd %d started", sockfd);
    return ESP_OK;
}

void session_close_callback(httpd_handle_t hd, int sockfd)
{
    ESP_LOGI(TAG, "Session with sockfd %d ended", sockfd);
    close(sockfd);
}
